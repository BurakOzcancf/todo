...
const Main = () => {
    ...
    function addToItem(e) {
        e.preventDefault()
        if (input !== "") {
            setList([...list, { id: Date.now(), names: input, line: false, edit: false }])
            setInput("")
            return;
        }
    }
        ...
        <section className='ToDo' >
            ...
            <section className='ToDo__filters'>
                <fieldset>
                    ...
                    />
                    <label
                        className={formData.filter === "" ? "ToDo__label--active" : "ToDo__label--inactive"}
                        htmlFor="all">
                        Show All
                    </label>
                    <input
                    ...
                    />
                    <label
                        className={formData.filter === "false" ? "ToDo__label--active" : "ToDo__label--inactive"}
                        htmlFor="completed">
                        Completed
                    </label>
                    <input                    
                    ...
                    />            
                    <label
                        className={formData.filter === "true" ? "ToDo__label--active" : "ToDo__label--inactive"}
                        htmlFor="active">
                        Active
                    </label>
                </fieldset>
                ...
                <div className="ToDo__menu">
                    <button
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 128 512" height={"22px"} width={'22px'}>
                            <path d="M64 360C94.93 360 120 385.1 120 416C120 446.9 
            ...
            {list.filter(item => item.line.toString() !== formData.filter).map(item =>
                <div
                    key={item.id}
                    className={item.line ? "ToDo__item--completed" : "ToDo__item--actives"}>
                    
                    <button
                        className='ToDo__toggle'
                        onClick={() => setList(list.map(num => num.id === item.id
                            ? { ...num, line: !num.line }
                            : num))}
                    >
                        {item.line &&
                            <svg xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 512 512" width={"22px"}>
                                <path d="M0 256C0 114.6 114.6 0 256 0C397.4 0 512 
                    ...
                    {(item.edit&& !item.line) ?
                        <input
                            ...
                            type="text"
                            value={item.names}
                            onChange={(e) => setList(list.map(num => num.id === item.id
                                ? { ...num, names: e.target.value }
                                : num))}
                        />
                        : <h1 className='ToDo__input--item'> {item.names} </h1>
                    }
                    <div className="ToDo__edit--wrapper">
                        <button
                        className='ToDo__edit'
                        onClick={() =>
                            setList(list.map(num => num.id === item.id ?
                                { ...num, edit: !num.edit }
                                : num))}
                    >
                    ...
                    <button
                        ...
                        onClick={() => {!item.line && setList(list.filter(el => el.id !== item.id))} }
                    >
                        <svg xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 320 512" height={"22px"} width={'22px'}>
                            <path d="M310.6 361.4c12.5 12.5 12.5 32.75 0 45.25C304.4
